...
**After obtaining the key:** 3. Ask user where they want to store the key (options):
...
miaoying update-vote --config ./update-vote-config.json
miaoying update-chacha --config ./update-chacha-config.json
```
...
**重要提示**：当需要配置高级选项时（如时间限制、位置限制、名单模式、WiFi限制等），请使用 `--config` 加载配置文件。CLI 参数只支持基础选项，高级配置请参考 `prompts/ai-form-prompt.md`。
...
# 使用配置文件更新统计
miaoying update --id <tongji-id> --config ./update-tongji.json
```
...
# ========== 更新 ==========
# 更新统计
miaoying update --id <tongji-id> --title "新标题" --close true

# 设置允许重复打卡
miaoying update --id <tongji-id> --repeat true

# 使用配置文件更新统计
cat > update-tongji.json << 'EOF'
{
  "title": "更新后的标题",
  "content": "更新后的描述",
  "isClose": true,
...
# 更新投票
miaoying update-vote --id <toupiao-id> --title "新标题" --publish-result false

# 更新查查
miaoying update-chacha --id <chacha-id> --title "新标题"
